...
#define __FUNCTION_END_TIME__(r)                                               \
  do {                                                                         \
    LOG_INFO("{}({}) total time({}) ms.", __FUNCTION__, r,                     \
             GetTickCount64() - _____begin_time_____);                         \
  } while (0);
...
#define __CONSOLE_CODE_SEGMENT_END__(name)                                     \
  std::cout << name << ": total("                                              \
            << GetTickCount64() - _____console_code_segment_begin_time_____    \
            << ")(ms)" << std::endl
...
#define RECT_EQUAL(rt1, rt2)                                                   \
  ((rt1.left == rt2.left) && (rt1.top == rt2.top) &&                           \
...
#ifndef SK_DELETE_PTR_BUFFER
#define SK_DELETE_PTR_BUFFER(p)                                                \
  do {                                                                         \
    if (!p)                                                                    \
      break;                                                                   \
    delete[] p;                                                                \
