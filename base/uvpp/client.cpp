...
void uv__udp_alloc_cb(uv_handle_t* handle,
	size_t suggested_size,
	uv_buf_t* buf);
void uv__udp_recv_cb(uv_udp_t* handle,
	ssize_t nread,
	const uv_buf_t* buf,
...
ServerType Client::ServerTypeGet() const {
	std::lock_guard<std::mutex> lock{ *m_Mutex };
	return Protocol::GetServerType(__gpService->ConfigGet()->GetServiceType());;
}
SessionType Client::SessionTypeGet() const {
	std::lock_guard<std::mutex> lock{ *m_Mutex };
	return Protocol::GetSessionType(__gpService->ConfigGet()->GetServiceType());
}
AddressType Client::AddressTypeGet() const {
	std::lock_guard<std::mutex> lock{ *m_Mutex };
	return Protocol::GetAddressType(__gpService->ConfigGet()->GetServiceType());
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Client::MainProcess(void* arg) {
	...
	do {
		...
		const unsigned long long time_current_ms = stl::Time::TimeStampUTC<std::chrono::milliseconds>();
		const unsigned long long time_reconnection_interval = time_current_ms - prev_time_ms_reconn;

		do {
			if (time_reconnection_interval < __gpService->ConfigGet()->ClientReconnectionIntervalMS())
				break;
			...
			case SessionStatus::DISCONNECTED: {
				...
				std::string connection_address = __gpService->ConfigGet()->Address();

				Connect(__gpService->ConfigGet()->Address(), connect, client);
				LOG_OUTPUT("Preparing to connect to the service ({}).", __gpService->ConfigGet()->Address());
			}break;
			case SessionStatus::FORCECLOSE: {
				prev_time_ms_keepalive = 0;
				dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientForceClose(client ? SESSION_PTR(client) : nullptr);
				LOG_OUTPUT("Disconnect from the service ({}).", __gpService->ConfigGet()->Address());
		...
		if (status == SessionStatus::CONNECTED && client) {
			...
			do {
				...
				pClient->m_WriteBufferQ.iterate_clear(
					[&](const auto& cmd, auto& data, bool&, bool& itclear) {
						pSession->Write(static_cast<unsigned long>(cmd), data.data(), data.size());
						itclear = true;
					});
			} while (0);
			...
			do {//!@ keepalive
				...
				if (time_current_ms - prev_time_ms_keepalive < __gpService->ConfigGet()->KeepAliveTimeMS())
					break;
				prev_time_ms_keepalive = time_current_ms;
				if (!pSession)
					break;
				...
				if (!pSession->Write(static_cast<unsigned long>(CommandType::KEEPALIVE), keepalive_send_data.data(), keepalive_send_data.size()))
					pSession->ForceClose();
			} while (0);
			...
			do {//!@ write
				write_req_t* req = pSession->Write(
					[&](std::string& message) {
						Buffer buf(message);
						dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientSessionWriteHookCb(pSession, dynamic_cast<IBuffer*>(&buf));
				...
				if (0 != uv_write(&req->write, reinterpret_cast<uv_stream_t*>(req->handle), &req->buf, 1, Protocol::uv_write_cb)) {
					...
					dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientError(pSession, SystemErrorno::E_STREAM_SEND, dynamic_cast<IBuffer*>(&buf));
					SK_DELETE_PTR(req);
					pSession->ForceClose();
					break;
				}
			...
			do {//read
				...
				if (!Protocol::UnMakeStream(read_data, head, message)) {
					...
					dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientError(pSession, SystemErrorno::E_STREAM_RECV, dynamic_cast<IBuffer*>(&buf));
					pSession->ForceClose();
					break;
				}
				Buffer msg(message);
				dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientMessageCb(pSession, head.Command(),
					dynamic_cast<IBuffer*>(&msg));
				{
					CommandType cmdReply = CommandType::UNKNOWN;
					Buffer messageReply;
					dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientMessageReceiveReplyCb(pSession, head.Command(),
						dynamic_cast<IBuffer*>(&msg), cmdReply, dynamic_cast<IBuffer*>(&messageReply));
					if (CommandType::UNKNOWN != cmdReply) {
						if (!pSession->Write(static_cast<unsigned long>(cmdReply), dynamic_cast<IBuffer*>(&messageReply)))
							pSession->ForceClose();
					}
				}
				...
				case CommandType::WELCOME: {
					...
					Buffer replyWelcome;
					dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientWelcome(
						dynamic_cast<IBuffer*>(&buffer), reinterpret_cast<IBuffer*>(&replyWelcome));
					if (!replyWelcome.Empty()) {
						if (!pSession->Write(static_cast<unsigned long>(CommandType::HELLO), replyWelcome.GetData(), replyWelcome.GetDataSize())) {
							pSession->ForceClose();
						}
					}
				}break;
	...
	uv_main_uninit(arg, loop, client, connect);
	dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientExit();
}
bool Client::Connect(const std::string& address, uv_handle_t* connect, uv_handle_t* client) {
	...
	do {
		...
		case SessionType::TCP: {
			...
			if (!Protocol::MakeIPAddr(__gpService->ConfigGet()->Address(), sockaddr_buffer, pClient->AddressTypeGet()))
				break;
		...
		case SessionType::UDP: {
			...
			if (0 != uv_udp_connect(reinterpret_cast<uv_udp_t*>(client), reinterpret_cast<LPSOCKADDR>(sockaddr_buffer.data())))
				break;
			if (0 != uv_udp_recv_start(reinterpret_cast<uv_udp_t*>(client), uv__udp_alloc_cb, uv__udp_recv_cb))
				break;
			result = true;
		}break;
		}
...
void Client::ConnectCb(uv_connect_t* req, int status) {
	...
	do {
		...
		if (!pClient)
			break;
		if (status < 0) {
			dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientDisconnection(pSession);
			/*pClient->NotifyFirstLogicalEvent();*/
			break;
		}
		if (0 != uv_read_start(reinterpret_cast<uv_stream_t*>(pSession->Handle()), Protocol::uv_alloc_cb, uv__read_cb))
			break;
		dynamic_cast<Config*>(ServiceGet()->ConfigGet())->OnClientConnection(pSession);
		pSession->Status(SessionStatus::CONNECTED);
		LOG_OUTPUT("Connect to the service({}) complete.", __gpService->ConfigGet()->Address());
		success = true;
	} while (0);
	if (!success && pSession)
		pSession->ForceClose();
