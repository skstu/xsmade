...
std::string File::GetFormat(const std::string &input_path, const bool &lower) {
  std::string parent_path, filename, extension, stem;
  Path::ParsePathname(input_path, parent_path, filename, extension, stem);
  if (lower) {
...
std::u16string File::GetFormat(const std::u16string &input_path,
                               const bool &lower) {
  std::u16string parent_path, filename, extension, stem;
  Path::ParsePathname(input_path, parent_path, filename, extension, stem);
  if (lower) {
    std::transform(extension.begin(), extension.end(), extension.begin(),
                   ::tolower);
...
std::wstring File::GetFormat(const std::wstring &input_path,
                             const bool &lower) {
  std::wstring parent_path, filename, extension, stem;
  Path::ParsePathname(input_path, parent_path, filename, extension, stem);
  if (lower) {
    std::transform(extension.begin(), extension.end(), extension.begin(),
                   ::tolower);
