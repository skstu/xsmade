...
int String::ICompareA(const std::string& s1, const std::string& s2) {
	return LowerA(s1).compare(LowerA(s2));
}
int String::ICompareW(const std::wstring& s1, const std::wstring& s2) {
	return LowerW(s1).compare(LowerW(s2));
}
...
vector<string> String::StringSpilt(const string& input, const string& delim) {
	...
	do {
		...
		std::string escaped_delim = regex_replace(delim, regex(R"([\.\+\*\?\[\^\$\(\|])"), R"(\$&)");
...
String::WStringSplit(const wstring& input, const wstring& delim) {
	...
	do {
		...
		std::wstring escaped_delim = regex_replace(delim, wregex(LR"([\.\+\*\?\[\^\$\(\|])"), LR"(\$&)");
