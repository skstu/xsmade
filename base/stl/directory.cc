...
void Directory::EnumU16(const std::u16string& inputPath,
	std::map<std::u16string/*full path*/, std::u16string/*relative path*/>& dirs,
	std::map<std::u16string/*full path*/, std::u16string/*relative path*/>& files) {
	do {
		...
		for (const fs::directory_entry& entry : fs::recursive_directory_iterator(inputPath)) {
			...
			if (fs::is_directory(entry.path())) {
				dirs.emplace(relativePathStr, stl::Path::NormalU16(inputPath + u"\\" + relativePathStr));
			}
			else {
				files.emplace(relativePathStr, stl::Path::NormalU16(inputPath + u"\\" + relativePathStr));
			}
		}
	} while (0);
}
void Directory::EnumU8(
	...
	do {
		...
		for (const fs::directory_entry& entry : fs::recursive_directory_iterator(inputPath)) {
			...
			if (fs::is_directory(entry.path())) {
				dirs.emplace(relativePathStr, stl::Path::NormalU8(inputPath + "\\" + relativePathStr));
			}
			else {
				files.emplace(relativePathStr, stl::Path::NormalU8(inputPath + "\\" + relativePathStr));
			}
		}
	} while (0);
}
void Directory::EnumW(const std::wstring& inputPath,
	std::map<std::wstring/*full path*/, std::wstring/*relative path*/>& dirs,
	std::map<std::wstring/*full path*/, std::wstring/*relative path*/>& files) {
	do {
		...
		for (const fs::directory_entry& entry : fs::recursive_directory_iterator(inputPath)) {
			...
			if (fs::is_directory(entry.path())) {
				dirs.emplace(relativePathStr, stl::Path::NormalW(inputPath + L"\\" + relativePathStr));
			}
			else {
				files.emplace(relativePathStr, stl::Path::NormalW(inputPath + L"\\" + relativePathStr));
			}
		}
	} while (0);
}
