...
bool FFmpeg::Start(const std::vector<std::string> &cmdline, const bool &show) {
  do {
    if (open_.load())
      break;
    std::wstring main;
    std::u16string current_dir = System::GetCurrentProcessPathU16();
    main = Conv::u16_to_ws(current_dir + uR"(\components\ffmpeg\ffmpeg.exe)");
    ...
    if (!ffmpegCmd.empty())
      ffmpegCmd.pop_back();
    if (!CreateProcessW(main.c_str(),                     // 应用程序名称
                        Conv::u8_to_ws(ffmpegCmd).data(), // 命令行
                        nullptr,                          // 进程安全属性
                        nullptr,                          // 线程安全属性
                        TRUE,                             // 继承句柄
