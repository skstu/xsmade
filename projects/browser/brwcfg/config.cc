...
void Config::Init() {
  ...
  do {
    ...
    path_.root = stl::Path::Parent(
        stl::Path::PathnameToPath(Conv::u8_to_u16(u8Dllpath)));
    path_.temp_dir = path_.root + u"/temp";
    ...
    configure_ = new IConfigure(stl::File::ReadFile(path_.configure_path));
    ...
    path_.configure_cache_path = path_.configure_cache_dir + u"/configure.json";
...
const brwcfg::ISettings &Config::GetSettings() const {
  std::lock_guard<std::mutex> lock(*mtx_);
  return *settings_;
}
const brwcfg::IConfigure &Config::GetConfigure() const {
  std::lock_guard<std::mutex> lock(*mtx_);
  return *configure_;
}
//////////////////////////////////////////////////////////////////////////////////////////
