...
IBackground::IBackground(wxWindow *parent, wxWindowID id, const wxString &title,
    ...
    : IFrame(parent, id, title, pos, size,
             /*wxFRAME_SHAPED | wxNO_BORDER | wxFRAME_NO_TASKBAR*/ wxNO_BORDER |
  ...
  SetTransparent(0xFF);
  //::SetWindowPos(GetHWND(), HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE |
  //: SWP_NOSIZE);
}
...
void IBackground::OnSize(wxSizeEvent &event) {
  do {
    HWND hChromium = GetTopWindow(GetHWND());
    if (!hChromium)
      break;
    tagRECT rtChromiumWindowRect;
    tagRECT rtChromiumClientRect;
    ::GetClientRect(hChromium, &rtChromiumClientRect);
    ::GetWindowRect(hChromium, &rtChromiumWindowRect);
    long chromiumBorderWidth =
        (rtChromiumWindowRect.right - rtChromiumWindowRect.left -
         rtChromiumClientRect.right) /
        2;
    /*chromiumBorderWidth -= 1;*/
    wxRect clientRect = GetClientRect();

    SetWindowPos(hChromium, nullptr, clientRect.x - chromiumBorderWidth,
                 clientRect.y - chromiumBorderWidth,
                 clientRect.width + chromiumBorderWidth * 2,
                 clientRect.height + chromiumBorderWidth * 2,
