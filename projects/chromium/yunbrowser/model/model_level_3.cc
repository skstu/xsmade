...
bool ModelLevel3::GetModel(chromium::xsiumio::IXSiumio &model) {
  bool result = false;
  ...
  do {
    ...
    // model.proxy.bridge_proxy_pass.clear();
...
bool ModelLevel3::GetProxyInfo(chromium::xsiumio::IXSiumio &xsio,
                               std::string &chromiumProxyUrl,
                               std::string &curlProxyUrl,
                               const bool &useCache) {
  bool result = false;
  std::lock_guard<std::mutex> lock(*mtx_);
  const std::string userDynamicProxyString = GetUserDynamicProxyString();
  if (xsio.bridge.IsEnable()) {

  } else {
    chromiumProxyUrl = "socks5://" + userDynamicProxyString;
    curlProxyUrl = "http://" + userDynamicProxyString;
  }

  // chromiumProxyUrl = "socks5://martell:5858668@127.0.0.1:15668";
  // std::string userDynamicProxyString = GetUserDynamicProxyString();
  // if (!xsio.proxy.bridge_proxy_pass.empty()) {
  //   //curlProxyUrl = xsio.proxy.bridge_proxy_pass;
  //   auto f_socks5h = curlProxyUrl.find("socks5h://");
  //   auto f_socks5 = curlProxyUrl.find("socks5://");
  //   auto f_http = curlProxyUrl.find("http://");
  //   if (f_socks5h != std::string::npos) {
  //     curlProxyUrl =
  //         curlProxyUrl.replace(f_socks5h, strlen("http://"), "http://");
  //   } else if (f_socks5 != std::string::npos) {
  //     curlProxyUrl =
  //         curlProxyUrl.replace(f_socks5, strlen("http://"), "http://");
  //   }
  // } else {
  //   curlProxyUrl = "http://" + userDynamicProxyString;
  // }

  // std::string bridgeProxy = xsio.proxy.bridge_proxy_pass.empty()
  //                               ? userDynamicProxyString
  //                               : xsio.proxy.bridge_proxy_pass;
  // SetBridgeStartupArgs(bridgeProxy, bridge_startup_args_,
  //                      xsio.proxy.bridge_proxy_pass);
  //   curlProxyUrl = xsio.proxy.bridge_proxy_pass;
  ...
  //   if (f_socks5h != std::string::npos) {
  //     curlProxyUrl =
  //         curlProxyUrl.replace(f_socks5h, strlen("http://"),
  //         "http://");
  //   } else if (f_socks5 != std::string::npos) {
  //     curlProxyUrl = curlProxyUrl.replace(f_socks5,
  //     strlen("http://"), "http://");
  //   }

  // std::cout << "bridge: " << xsio.proxy.bridge_proxy_pass << std::endl;
  //  std::cout << "curl-x: " << curlProxyUrl << std::endl;
  //  std::cout << "chromium: " << chromiumProxyUrl << std::endl;

  // bridge_startup_args = OpenSSL::Base64Encode(bridge_proxy_pass_tmp, false);
  return result;
}

const std::string &ModelLevel3::GetBridgeStartupArgs() const {
  std::lock_guard<std::mutex> lock(*mtx_);
