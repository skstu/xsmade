...
void IConfigure::Server::operator<<(const rapidjson::Value &serverObj) {
  do {
    ...
    if (serverObj.HasMember("http") && serverObj["http"].IsString() &&
        serverObj["http"].GetStringLength() > 0) {
      http = serverObj["http"].GetString();
    }
  } while (0);
}
  ...
  do {
    ...
    do { //!@ .startup_urls
      ...
      for (auto it = doc["startup_urls"].Begin();
        ...
        if (it->GetStringLength() <= 0)
          continue;
        std::string tmpv(it->GetString());
        if (std::find(startup_urls.begin(), startup_urls.end(), tmpv) !=
            startup_urls.end())
          continue;
        startup_urls.emplace_back(tmpv);
      }
    } while (0);
    do { //!@ .frame
    ...
    do { //!@ .startup_args
      ...
      for (auto it = doc["startup_args"].Begin();
        ...
        if (it->GetStringLength() <= 0)
          continue;
        startup_args.emplace_back(it->GetString());
      }
    } while (0);
    do { //!@ .jss
