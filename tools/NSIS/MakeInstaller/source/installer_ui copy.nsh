...
Function DUIPage
	...
	libnswxskin::InitInstallPage "$PLUGINSDIR\" "$PLUGINSDIR\${INSTALL_LICENCE_FILENAME}" "${INSTALL_DEFALT_SETUPPATH}"
    Pop $hInstallDlg
	#设置安装包的标题及任务栏显示  
...
Function BindUIControls
    ...
    #nsxsskin::BindCallBack $hInstallDlg "OnEditDirTextChange" $0
...
Function CreateDesktopShortcut
	...
	Sleep 10
	CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\client\${EXE_NAME}"
...
Function CreatePowerOn
	SetShellVarContext all
	CreateShortCut "$SMSTARTUP\${PRODUCT_NAME}.lnk" "$INSTDIR\client\${EXE_NAME}"
	SetShellVarContext current
FunctionEnd
...
;     System::Int64Op $3 / $2
;     Pop $0
; 	nsxsskin::SetControlAttribute $hInstallDlg "install_progress" "value" "$1"
; 	;nsxsskin::SetControlAttribute $hInstallDlg "install_progress" "text" "正在安装 $0%"  
;     ${If} $1 == $2
;         nsxsskin::SetControlAttribute $hInstallDlg "install_progress" "value" "100"	
;         ;nsxsskin::SetControlAttribute $hInstallDlg "install_progress" "text" "正在安装 100%"        
;     ${EndIf}
; FunctionEnd
...
Function OnBtnInstallClick
	#MessageBox MB_OK|MB_ICONINFORMATION "DEBUG 0"
    ...
    ; Pop $1	
	; ${If} $1 == "0"
    ;     StrCpy $R5 "您选择的安装路径无效，请重新选择!"
	; 	Call ShowMsgBox
	; 	Goto funcend
    ; ${EndIf}

	#nsxsskin::SetControlAttribute "nsisSetInstallDir" "$INSTDIR\"
	#nsxsskin::SetControlAttribute "nsisSetProgramMain" "$INSTDIR\client\${EXE_NAME}"

	#MessageBox MB_OK|MB_ICONINFORMATION "DEBUG 1"
...
Function un.DUIPage
	...
	#nsxsskin::InitUnInstallPage "$PLUGINSDIR\" "$0"
